...
function ChatWindow({ activeChat, currentUser, onMessageSent, onOpenUserProfile, backendUrl, onlineUsers, onCloseChat, typingUsers }) {
    ...
    const loadMoreMessages = useCallback(async () => {
        ...
        try {
            ...
            const res = await API.get(`/messages/${activeChat.username}?before=${oldestId}&limit=100`);
    ...
    if (!activeChat) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center bg-[#0a0a12] text-center px-6">
                <div className="w-20 h-20 bg-[#1a1a25] rounded-full flex items-center justify-center mb-4">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-10 h-10 text-blue-400/50">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                    </svg>
                </div>
                <h3 className="text-lg font-semibold text-neutral-100 mb-1">ChatNow</h3>
            ...
            <div className="px-3 md:px-6 py-3 md:py-4 bg-[#111118] border-b border-[#1e1e2a] flex items-center gap-2 md:gap-3 flex-shrink-0">
                ...
                >
                    <div className="relative flex-shrink-0">
                        ...
                        {isOnline && (
                            <span className="absolute bottom-0 right-0 w-2.5 h-2.5 md:w-3 md:h-3 bg-green-500 border-2 border-[#111118] rounded-full"></span>
                        )}
                    </div>
            ...
            >
                ...
                {hasMore && !loadingMore && messages.length > 0 && (
                    <button
                        ...
                        className="w-full text-center text-xs text-blue-400 py-2 hover:underline cursor-pointer"
                    >
                        Load older messages
                    </button>
                )}
                ...
                {loading ? (
                    <div className="flex items-center justify-center h-full">
                        <div className="relative flex items-center justify-center">
                            <div className="absolute w-28 h-28 rounded-full animate-[spin_2.5s_linear_infinite]"
                                style={{
                                    background: 'conic-gradient(from 0deg, transparent 0%, #0084FF15 25%, #0084FF40 50%, #0084FF15 75%, transparent 100%)',
                                }}
                            />
                            <div className="absolute w-36 h-36 rounded-full animate-[spin_4s_linear_infinite_reverse]"
                                style={{
                                    background: 'conic-gradient(from 180deg, transparent 0%, #0084FF08 25%, #0084FF20 50%, #0084FF08 75%, transparent 100%)',
                                }}
                            />
                            <div className="absolute w-24 h-24 rounded-full bg-blue-500/5 animate-[pulse_2s_ease-in-out_infinite]" />
                            <div className="absolute w-26 h-26 rounded-full border border-blue-500/10" />
                            <div className="relative z-10 animate-[scaleIn_0.6s_ease-out_forwards]">
                                <img
                                    src="/chatnow new logo png.png"
                                    alt="Loading"
                                    className="w-16 h-16 object-contain drop-shadow-[0_0_20px_rgba(0,132,255,0.2)]"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center">
                        ...
                        <p className="text-xs text-neutral-600 mt-1">Say hello to {activeChat.name}!</p>
                    </div>
                ) : (
                    <>
                        {messages.map((msg) => (
